...
class OmegaResultsPlotter:
    ...
    def plot_flow_rate_vs_omega(self, output_file: Optional[Path] = None) -> None:
        ...
        ax1.errorbar(self.omega_values, self.flow_rates, yerr=self.flow_rate_errors,
                    ...
                    label='Flow rate with std error')

        # Mark maximum
        max_idx = np.argmax(self.flow_rates)
        max_omega = self.omega_values[max_idx]
        max_q = self.flow_rates[max_idx]
        max_q_err = self.flow_rate_errors[max_idx]

        ax1.plot(max_omega, max_q, '*', markersize=20, color='red',
                label=f'Maximum: ω = {max_omega:.0f} s⁻¹, Q = {max_q:.4f} ± {max_q_err:.4f} p/s')

        ax1.set_xlabel('Vibration frequency ω (s⁻¹)', fontsize=13, fontweight='bold')
        ax1.set_ylabel('Flow rate Q (particles/s)', fontsize=13, fontweight='bold')
        ax1.set_title('Flow Rate vs Vibration Frequency', fontsize=15, fontweight='bold')
